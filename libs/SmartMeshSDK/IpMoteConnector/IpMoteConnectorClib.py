...
from . import IpMoteConnector as IpMoteConnectorOriginal

#============================ decide whether can run ==========================

CLIB_PATHS = [
    os.path.join('libdn_ipmt'),
    os.path.join('..','..','..','sm_clib','sm_clib','libdn_ipmt'),
]
if os.name!='nt':
    for P in CLIB_PATHS:
        P += '.so'
...
class NullHandler(logging.Handler):
    def emit(self, record):
        pass
log = logging.getLogger('IpMoteConnector')
...
ipmt_notif_cbt                    = CFUNCTYPE(c_void_p, c_uint8, c_uint8)
ipmt_notif                        = None

ipmt_reply_cbt                    = CFUNCTYPE(c_void_p, c_uint8)
ipmt_reply                        = None

ipmt_status_cbt                   = CFUNCTYPE(c_int, c_uint8)
ipmt_status                       = None

INT                               = IpMoteDefinition.IpMoteDefinition.INT
INTS                              = IpMoteDefinition.IpMoteDefinition.INTS
BOOL                              = IpMoteDefinition.IpMoteDefinition.BOOL
HEXDATA                           = IpMoteDefinition.IpMoteDefinition.HEXDATA
...
class IpMoteConnector(ApiConnector,threading.Thread):
    ...
    def dn_getParameter_moteInfo(self, ):
        ...
        res['RC'] = 0
        return IpMoteConnectorOriginal.IpMoteConnector.Tuple_dn_getParameter_moteInfo(**res)
    ...
    def _ipmt_status(self,newStatus):
        '''
        \brief Called by C library when connection status is changed.
        '''
        log.debug('_ipmt_status newStatus={0}'.format(newStatus))
        
        return 0
    
    # data notifications
    
    def _ipmt_reply(self,cmdId):
        '''
        \brief Called by C library when a reply is received.
        '''
        log.debug('_ipmt_reply cmdId={0} (0x{0:02x})'.format(cmdId))
    ...
    def _ipmt_notif(self,cmdId,subcmdId):
        ...
        '''
        log.debug(
            '_ipmt_notif cmdId={0} subcmdId={1} notifBuf={2}'.format(
                cmdId,
                subcmdId,
